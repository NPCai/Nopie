...
for batch in range(batchRange): 
	...
	seq_tensor = torch.empty((len(batchSeqIn), seq_lengths.max(), EMBED_DIM)).float().to(device)
	tgt_tensor = torch.empty((len(batchSeqIn), out_lengths.max())).to(device)
	embed_tensor = torch.empty((len(batchSeqIn), out_lengths.max(), EMBED_DIM)).to(device)
